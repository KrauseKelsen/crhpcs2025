...
void initialize_grid(Kokkos::View<double**> grid, int nx, int ny) {
    ...
    Kokkos::parallel_for("initialize_grid", nx * ny, KOKKOS_LAMBDA(const int idx) {
        int i = idx / ny;
        int j = idx % ny;
...
* @param[in] ny Number of columns in the grid.
*/
void diffuse_heat(Kokkos::View<double**> current, 
                  Kokkos::View<double**> next, 
                  int nx, int ny) {
    Kokkos::parallel_for("Diffuse Heat", Kokkos::RangePolicy<>(1, nx-1), KOKKOS_LAMBDA(const int i) {
    ...
    Kokkos::parallel_for("Apply Boundary Conditions", Kokkos::RangePolicy<>(1, nx-1), KOKKOS_LAMBDA(const int i) {
        next(i, 0) = 100.0;
        next(i, ny-1) = 100.0;
    });
    Kokkos::parallel_for("Apply Boundary Conditions", Kokkos::RangePolicy<>(1, ny-1), KOKKOS_LAMBDA(const int j) {
        next(0, j) = 100.0;
        next(nx-1, j) = 100.0;
    });
}
...
/*
* @brief This function outputs the current state of the heat distribution to a text file.
*
* This function creates a text file named "heat_<step>.txt" where <step> is the current time step number.
* It writes the temperature values of each grid point in a row-major order.
* The output is formatted such that each row corresponds to a line in the text file.
*
...
*/
void output_heat_distribution(const Kokkos::View<double**> current, int nx, int ny, int step) {
    std::ofstream file;
    std::string filename = "heat_" + std::to_string(step) + ".txt";
    auto host_current = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), current);
    file.open(filename);
    if (file.is_open()) {
        for(int i = 0; i < nx; ++i) {
            for(int j = 0; j < ny; ++j) {
...
*/
void output_vtk(const Kokkos::View<double**> current, int nx, int ny, int step) {
    std::ofstream file;
    ...
    if (file.is_open()) {
        ...
        for (int j = 0; j < ny; ++j) {  // Loop over y direction first for VTK file format
...
int main(int argc, char **argv) {
    ...
    }
    {
        ...
        for (int step = 0; step < steps; ++step) {
            ...
            // Swap the views to use the updated heat distribution in the next iteration
